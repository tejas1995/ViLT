...
def make_arrow(root, dataset_root):
    ...
    for split, annots in zip(
        ["train"], [annotations_train2014],
    ):
        _annot = annotations[split]
        for q in tqdm(annots):
            answers = [x['answer'] for x in q['answers']]
            top_answer = max(answers, key=answers.count)
            all_major_answers.append(top_answer)

    all_major_answers = [normalize_word(word) for word in tqdm(all_major_answers)]
    counter = {k: v for k, v in Counter(all_major_answers).items() if v >= 1}
    ans2label = {k: i for i, k in enumerate(counter.keys())}
    label2ans = list(counter.keys())
    ans2label['NOTA'] = len(label2ans)
    label2ans.append('NOTA')
    print(len(label2ans))
    sys.exit(0)
